...
##################################################################
#Code I added on.. For simple parrallization purposes
##################################################################
num_samples=`expr $ncol - "2"`
samples_per_node=`expr $num_samples / $total_jobs + "1"`
start_sample=`expr $samples_per_node \* $job_number + "3"`
temp_num=`expr $job_number + "1"`
end_sample=`expr $samples_per_node \* $temp_num + "2"`
if [ $end_sample -gt $ncol ]; then
  end_sample=$ncol
fi 
new_cols=$(eval echo "{$start_sample..$end_sample}")
echo $new_cols
