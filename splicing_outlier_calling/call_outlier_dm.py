...
def extract_raw_gene_jxn_data_strucutre(tissue_specific_jxn_file,used_samples):
	...
	for line in f:
		...
		for gene in genes:
			#add jxn's read counts to this gene
			gene_jxn_data_structure = add_jxn_to_gene_jxn_data_structure(gene_jxn_data_structure,gene,jxn_read_counts)
	f.close()
...
def outlier_analysis_dm(X,samples):
	...
	sample_distances = adaptive_sample(alpha,read_counts[outlier_sample_num],max_observed_distance)
...
def call_outliers_with_dirichlet_multinomial(tissue_specific_outlier_file_root,gene_jxn_data_structure,all_samples,start_number,end_number):
	...
	t_pvalue = open(tissue_specific_outlier_file_root + '_pvalue.txt','w') #file handle for matrix of pvalues 
	#Write headers for output files
	t_MD.write('GENE_ID\t' + '\t'.join(all_samples) + '\n')
	t_pvalue.write('GENE_ID\t' + '\t'.join(all_samples) + '\n')
